...
def simulate_ai_attack(prompt: str, delay: int):
    """Simulate an attack generated by AI."""
    print("=" * 60)
    print(f"CyberMaps AI Attack Simulation")
    print(f"Prompt: {prompt}")
    print("Generating scenario... (this may take a few seconds)")
    print("=" * 60)

    try:
        # 1. Generate Alerts
        gen_response = requests.post(f"{BASE_URL}/api/simulation/generate", json={"prompt": prompt})
        ...
        print(f"✓ Generated {len(alerts)} alerts. Starting simulation...")
        print()
...
Examples:
  ...
  python simulate_attack.py --scenario aws_breach --delay 5
    ...
    parser.add_argument(
        '--prompt',
        type=str,
        help='Use AI to generate a scenario based on this prompt (overrides --scenario)'
    )
    
    args = parser.parse_args()
    
    if args.prompt:
        simulate_ai_attack(args.prompt, delay=args.delay)
    else:
